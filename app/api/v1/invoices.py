...
from app.schemas.invoice_preview_schema import InvoicePreviewCreate, InvoicePreviewRead, InvoicePreviewUpdate
from app.services.invoice_preview_service import InvoiceService

router = APIRouter()

@router.post("/", response_model=InvoicePreviewRead, status_code=status.HTTP_201_CREATED)
def create_invoice(payload: InvoicePreviewCreate, db: Session = Depends(db_session)):
...
@router.get("/{invoice_id}", response_model=InvoicePreviewRead)
...
@router.put("/{invoice_id}", response_model=InvoicePreviewRead)
def update_invoice(invoice_id: int, payload: InvoicePreviewUpdate, db: Session = Depends(db_session)):
